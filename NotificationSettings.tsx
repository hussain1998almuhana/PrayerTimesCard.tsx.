...
import { 
  ...
  Square, BellRing, ChevronDown, ChevronUp, Minus, Plus, Info, FileText
} from 'lucide-react';
...
const PRAYERS = ['Fajr', 'Dhuhr', 'Asr', 'Maghrib', 'Isha', 'FajrTaqibat'] as const;
...
const NotificationSettings: React.FC<NotificationSettingsProps> = ({ onBack, lang }) => {
  ...
  const [configs, setConfigs] = useState<Record<string, PrayerNotificationConfig>>(() => {
    const saved = localStorage.getItem('prayer_notifications_final_v6');
    ...
    PRAYERS.forEach(p => {
      initialConfigs[p] = { 
        enabled: true, 
        offset: p === 'FajrTaqibat' ? 10 : 0, 
        sound: 'osama',
        customText: p === 'FajrTaqibat' ? (lang === 'ar' ? "أصبحت اللهم معتصماً بذمامك المنيع..." : "O Allah, I have entered the morning clinging to Your secure protection...") : ""
      };
    });
  ...
  useEffect(() => {
    localStorage.setItem('prayer_notifications_final_v6', JSON.stringify(configs));
  }, [configs]);
  ...
  const getPrayerName = (p: string) => {
    switch (p) {
        ...
        case 'FajrTaqibat': return lang === 'ar' ? 'تعقيبات صلاة الفجر' : 'Fajr Supplications';
        default: return p;
    }
  };
    ...
    <div className="min-h-screen flex flex-col animate-in fade-in duration-500 bg-emerald-50/30 dark:bg-slate-950">
      <header className="bg-white/80 backdrop-blur-xl text-emerald-900 p-6 pt-12 rounded-b-[3.5rem] sticky top-0 z-50 flex items-center border-b border-emerald-100 shadow-xl dark:bg-slate-900/80 dark:border-white/5 dark:text-white">
        ...
        <div className="flex-1 text-center">
          <h2 className="text-xl font-black spiritual-heading">{t.notificationSettings}</h2>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6 pb-40">
            ...
            <div key={prayer} className={`bg-white/90 backdrop-blur-xl rounded-[2.5rem] shadow-xl border overflow-hidden transition-all duration-500 dark:bg-slate-900/60 ${activePrayer === prayer ? 'border-emerald-500/50 scale-[1.02]' : 'border-transparent'}`}>
              ...
              >
                  <div className="flex items-center gap-4">
                      <div className={`p-4 rounded-2xl shadow-inner transition-all ${configs[prayer].enabled ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-slate-100 text-slate-300 dark:bg-slate-800'}`}>
                          {prayer === 'FajrTaqibat' ? <FileText size={24} /> : <Bell size={24} />}
                      </div>
                      <div className={lang === 'ar' ? 'text-right' : 'text-left'}>
                          <p className={`text-lg font-black leading-tight spiritual-heading ${configs[prayer].enabled ? 'text-emerald-900 dark:text-white' : 'text-slate-400'}`}>{getPrayerName(prayer)}</p>
              ...
              {activePrayer === prayer && (
                  <div className="px-6 pb-10 space-y-8 animate-in slide-in-from-top-3 duration-300 border-t border-emerald-50 dark:border-white/5 pt-6">
                      ...
                      {/* إعدادات النص المخصص لتعقيب الفجر */}
                      {prayer === 'FajrTaqibat' && (
                        <div className="space-y-4">
                           <div className="flex items-center gap-3 px-1 text-emerald-700 dark:text-amber-400">
                              <FileText size={20} />
                              <span className="text-xs font-black uppercase tracking-widest">{lang === 'ar' ? 'نص التعقيب المخصص' : 'Custom Supplication Text'}</span>
                           </div>
                           <textarea 
                             className="w-full bg-emerald-50/50 border border-emerald-100 rounded-[1.5rem] p-4 text-sm font-bold quran-text leading-relaxed text-emerald-900 dark:bg-white/5 dark:border-white/10 dark:text-white min-h-[120px] outline-none focus:ring-2 focus:ring-emerald-500"
                             value={configs[prayer].customText || ''}
                             onChange={(e) => updateConfig(prayer, 'customText', e.target.value)}
                             placeholder={lang === 'ar' ? 'اكتب نص التعقيب هنا لظهوره مع التنبيه...' : 'Enter supplication text here...'}
                           />
                        </div>
                      )}
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="bg-emerald-50/50 p-6 rounded-[2rem] border border-emerald-100 shadow-inner flex items-center justify-between gap-6 dark:bg-white/5 dark:border-white/5">
                           <button 
                             onClick={() => updateConfig(prayer, 'offset', Math.max(-60, configs[prayer].offset - 5))}
                             className="p-4 bg-white rounded-2xl text-emerald-600 shadow-md active:scale-90 transition-all dark:bg-slate-800 dark:text-amber-400"
                           >
                             <Minus size={24} strokeWidth={3} />
                           </button>
                           
                           <div className="text-center flex-1 flex flex-col items-center">
                              <span className="text-2xl font-black text-emerald-900 dark:text-white tabular-nums drop-shadow-sm">
                                {formatOffset(configs[prayer].offset)}
                              </span>
                              <div className="h-1 w-12 bg-emerald-500/20 rounded-full mt-2" />
                           </div>
                              ...
                              {(['none', 'maytham', 'osama', 'abather', 'amer', 'bassem', 'beep'] as const).map((s) => (
                                  ...
                                  >
                                      <div className="flex items-center gap-4">
                                        ...
                                        <span className="font-bold">{getSoundLabel(s)}</span>
                                      </div>
        ...
        <div className="p-8 rounded-[3rem] bg-emerald-950/5 border border-emerald-100 text-center dark:bg-white/5 dark:border-white/5">
           ...
           <p className="text-emerald-900/60 font-bold text-xs leading-relaxed dark:text-white/40">
             تمت إضافة تعقيبات صلاة الفجر المخصصة. يمكنك ضبط وقت التنبيه (مثلاً بعد ١٠ دقائق من الصلاة) ليظهر لك نص التعقيب الذي اخترته.
           </p>
        </div>
      </div>
    </div>
